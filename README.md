...
- **cron 写的是 `*/5`，但那只是请求，不是保证。** GitHub 在负载高时会延迟
  甚至静默跳过定时运行，调度越密越容易被丢。写 `*/5` 不吃亏所以保留，
  但别当成承诺。
- **实测节奏**（2026-08-22，连续观测 107 分钟，5 次自动运行全部成功）：
  ...
  最短 20 / 最长 35 / 平均 25 分钟。也就是说远达不到 5 分钟，但**明显好于
  每小时一次**。刚建仓那会儿有过一次 27 分钟才首跑的冷启动，之后就稳定了。
  仍然要知道：**补货如果十几分钟内被抢光，有可能错过**。
- 想要真正的 5 分钟，只能用长任务循环（`--interval 300`）常驻占着 runner，
  或者干脆跑在自己的机器上。这里是有意选择了不这么做。
- 脚本只读页面，不会加购、不会下单。
